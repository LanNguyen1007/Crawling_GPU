...
for n_page in range(1, 101):
    page = "https://www.newegg.com/GPUs-Video-Graphics-Cards/SubCategory/ID-48/Page-{}?Tid=7709".format(
        n_page
    )
    ...
    for i in range(len(soup.find_all("div", class_="item-container"))):
        ...
        try:
            brand = (
                soup.find_all("a", class_="item-brand")[i]
                .find_all("img")[0]
                .attrs["title"]
            )
        except:
        ...
        if len(soup.find_all("div", class_="item-branding")[i]) == 2:
            rating = (
                soup.find_all("div", class_="item-branding")[i]
                .find_all("a", class_="item-rating")[0]
                .attrs["title"]
                .split()[-1]
            )
            n_rating = (
        ...
        try:
            price = float(
                (
                    list(
                        soup.find_all("li", class_="price-current")[i].stripped_strings
                    )[1]
                    + list(
                        soup.find_all("li", class_="price-current")[i].stripped_strings
                    )[2]
        ...
        try:
            max_rslt = re.findall(
                "(\d+ x \d+)", soup.find_all("ul", class_="item-features")[i].text
            )[0]
        except:
            max_rslt = ""
        try:
            dp = re.findall(
                "DisplayPort: (\d+ x \S+ \d+.\d+\S)",
                soup.find_all("ul", class_="item-features")[i].text,
            )[0]
        except:
            dp = ""
        try:
        ...
        try:
            model = re.findall(
                "Model #: (\S+)", soup.find_all("ul", class_="item-features")[i].text
            )[0]
